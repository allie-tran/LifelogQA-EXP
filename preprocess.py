# coding=utf-8
# inspired by Junwei Liang junweil@cs.cmu.edu
# author: Allie Tran ly.tran2@mail.dcu.ie
# preprocess all usefull information into 3 set

"""
1. Giving the original memoryqa dataset, will generate a *_data.p, *_shared.p for each split. 
2. Tokenized question, answer, pointer to album will be in data.p, 
3. Wordcounter, album will be in shared.p
"""
...
def get_args():
    ...
    parser.add_argument("imgfeat", action="store", type=str,
                        help="/path/to img feat npz file")
    parser.add_argument("glove", action="store", type=str,
                        help="/path/to glove vector file")
...
def process_tokens(tokens):
    ...
    for token in tokens:
        ...
        newtokens.extend([one for one in re.split("[%s]" %
                                                  ("").join(l), token) if one != ""])
    return newtokens
...
def prepro_each(args, data_type, question_ids, start_ratio=0.0, end_ratio=1.0):
    ...
    # right now we don't do sentence tokenization # just for album_description
    def sent_tokenize(para): return [para]
    ...
    for idx, question_id in enumerate(tqdm(question_ids[start_idx:end_idx])):
        ...
        for albumId in qa['album_ids']:
            ...
            # remember how many times this album is used
        ...
        # this is for during testing, we need to reconstruct the answer in the original order
        yidxi = ci.index(qa['answer'])
    ...
    for albumId in tqdm(global_aids):
        ...
        temp['description'] = word_tokenize(
        ...
        # photo info
        temp['photo_titles'] = [word_tokenize(
            title) for title in album['photo_titles']]
        temp['photo_titles_c'] = [
            [list(tok) for tok in title] for title in temp['photo_titles']]
        ...
        if(debug):
            ...
            print("album when:%s,where:%s" %
                  (album['album_when'], album['album_where']))
    ...
    pickle.dump(data, open(os.path.join(
        args.outpath, "%s_data.p" % data_type), "wb"))
    pickle.dump(shared, open(os.path.join(
        args.outpath, "%s_shared.p" % data_type), "wb"))
...
def getTrainValIds(qas, validlist, testidlist):
    ...
    print("total trainId:%s,valId:%s,testId:%s, total qa:%s" %
          (len(trainIds), len(valIds), len(testIds), len(qas)))
...
if __name__ == "__main__":
    ...
    trainIds, valIds, testIds = getTrainValIds(
        args.qas, args.valids, args.testids)
